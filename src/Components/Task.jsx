  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://task-management-server-dun.vercel.app/task/${_id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
      ...
      <div className="card bg-base-100 shadow-xl border rounded-lg">
        <div className="card-body">
          <div className="flex justify-betwee gap-3">
            <h2 className="flex-grow text-lg py-1 px-2 font-medium text-black rounded-lg bg-[#EEE5FF]">
              {taskName}
            </h2>
            <button
          ...
          <div className="flex justify-between items-center">
            ...
            <div>
              <button
                className={`btn ${
                  status === "In Progress"
                    ? "bg-[#9BCEED] text-white"
                    : "bg-[#C3E3C3] text-[#2B3440]"
                }`}
